...
import threading     # to handle the continuous movement of the snake in the background. 
import random
...
def get_random_position():
    x=random.randint(10,390)     #random no. between 10 to 390
    y=random.randint(20,390)
    x=x-(x%10)             #to make that food in window
    y=y-(y%10)
    return (x,y)
...
root.protocol("WM_DELETE_WINDOW", on_closing)      # when the user attempts to close the window (by clicking the close button on the window title bar), the on_closing function should be called
root.bind('<Up>', up_fun)          # when the user presses the "Up" arrow key, the up_fun function will be executed
